    ...
    <div x-data="{ 
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-5 flex items-center justify-between border-b border-slate-100 flex-wrap gap-4">
                ...
                <button 
                    @click="openModal = true; isEdit = false; modalTitle = 'Tambah Pasien Baru'; idPasien = 'PS-' + Math.floor(Math.random() * 90000 + 10000); namaLengkap = ''; noTelepon = ''; tglLahir = ''; alamat = ''" 
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <td class="px-6 py-4 text-xs font-semibold text-slate-500">{{ $pasien['id'] }}</td>
                            <td class="px-6 py-4 text-xs font-bold text-slate-700">{{ $pasien['nama'] }}</td>
                            <td class="px-6 py-4 text-xs font-medium text-slate-600">{{ $pasien['no_telepon'] }}</td>
                            <td class="px-6 py-4 text-xs text-slate-600">{{ $pasien['tgl_lahir'] }}</td>
                            <td class="px-6 py-4 text-xs text-slate-500 truncate max-w-[200px]" title="{{ $pasien['alamat'] }}">{{ $pasien['alamat'] }}</td>
                            <td class="px-6 py-4 text-xs text-right">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <button 
                                        @click="if(confirm('Hapus pasien ini?')) alert('Data berhasil dihapus (Mocking Action)')"
